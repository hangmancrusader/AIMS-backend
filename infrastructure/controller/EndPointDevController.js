...
router.get("/getcount", async (req, res) => {
  try {
    const result = await EndPointDevice.getcount();
    if (result) {
      res.status(201).json(result);
    } else {
      res.status(404).json({ message: "Not found" });
...
router.get("/getperct", async (req, res) => {
  try {
    const result = await EndPointDevice.getperct();
    if (result) {
      res.status(201).json(result);
    } else {
      res.status(404).json({ message: "Not found" });
...
router.get("/getcost", async (req, res) => {
  try {
    const result = await EndPointDevice.getcost();
    if (result) {
      res.status(201).json(result);
    } else {
      res.status(404).json({ message: "Not found" });
